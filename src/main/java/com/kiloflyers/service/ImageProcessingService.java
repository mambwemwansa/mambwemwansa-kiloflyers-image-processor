...
public class ImageProcessingService {
  ...
  public List<AirtableRecord> fetchUnprocessedRecords() {
    String url = String.format("https://api.airtable.com/v0/%s/%s", new Object[] { this.airtableBaseId, this.airtableTableName });
    System.out.println("Airtable endpoint: " + url);
    HttpResponse<String> response = ((GetRequest)Unirest.get(url).header("Authorization", "Bearer " + this.airtableApiKey)).asString();
    String jsonResponse = (String)response.getBody();
    System.out.println("Raw Airtable Response: " + jsonResponse);
    if (response.getStatus() == 200) {
      ObjectMapper objectMapper = new ObjectMapper();
      try {
        AirtableResponse airtableResponse = (AirtableResponse)objectMapper.readValue(jsonResponse, AirtableResponse.class);
        List<AirtableRecord> records = airtableResponse.getRecords();
        for (AirtableRecord airtableRecord : records) {
          uploadNoBckImageToAirtable("https://wallpapers.com/images/featured/coolest-pictures-88c269e953ar0aw4.jpg", airtableRecord.getId());
          uploadToBeFramedImageToAirtable("https://wallpapers.com/images/hd/fantastic-image-of-falling-spaceman-40xgzkhbx0i01m7r.jpg", airtableRecord.getId());
          uploadFramedImageToAirtable("https://thumbs.dreamstime.com/b/trendy-neon-lighted-wallpaper-background-modern-design-contemporary-art-collage-sea-side-cityscape-bright-colors-207822161.jpg", airtableRecord.getId());
        } 
  ...
  public void removeBackground(AirtableRecord record) {
    ...
    String backgroundRemovedImageUrl = callRemoveBgApi(originalImageUrl, ((Image)originalImages.get(0)).getFilename());
  ...
  public void updateRecords(List<AirtableRecord> records) {
    ...
    try {
      ...
      HttpResponse<String> response = ((HttpRequestWithBody)((HttpRequestWithBody)Unirest.patch(url).header("Authorization", "Bearer " + this.airtableApiKey)).header("Content-Type", "application/json")).body(requestBody).asString();
      if (response.getStatus() == 200) {
        System.out.println("Records updated successfully: " + (String)response.getBody());
      } else {
        System.out.println("Error updating records: " + response.getStatus() + " " + (String)response.getBody());
      } 
  ...
  public void insertImageRecord(String imageUrl) {
    ...
    HttpResponse<String> response = ((HttpRequestWithBody)((HttpRequestWithBody)Unirest.post(url).header("Authorization", "Bearer " + this.airtableApiKey)).header("Content-Type", "application/json")).body(jsonBody).asString();
  ...
  public void uploadNoBckImageToAirtable(String imageUrl, String recordId) {
    try {
      ...
      String updateUrl = String.format("https://api.airtable.com/v0/%s/%s/%s", new Object[] { this.airtableBaseId, this.airtableTableName, recordId });
      HttpResponse<JsonNode> updateResponse = ((HttpRequestWithBody)((HttpRequestWithBody)Unirest.patch(updateUrl).header("Authorization", "Bearer " + this.airtableApiKey)).header("Content-Type", "application/json")).body(String.format("{\"fields\": {\"%s\": [{\"url\": \"%s\"}]}}", new Object[] { fieldId, imageUrl })).asJson();
      if (updateResponse.isSuccess()) {
        System.out.println("Record updated successfully: " + String.valueOf(updateResponse.getBody()));
      } else {
        System.err.println("Error updating record: " + updateResponse.getStatus() + " - " + String.valueOf(updateResponse.getBody()));
      } 
    } catch (Exception e) {
      e.printStackTrace();
    } 
